...
function refreshBlips()
	if playerData.job.name ~= nil and playerData.job.name == 'fork' then 	
		drawBlip(Config.locker, 280, "Heftruck kleedruimte")			 	
		drawBlip(Config.carSpawner, 479, "Heftruck spawnen")					
		drawBlip(Config.carDelete, 490, "Heftruck inleveren")				
	end
end
...
function goDeliver()
	ESX.ShowNotification('Lever deze lading af(geel stipje op je navigatie).')
	setGPS(taskPoints['deliver'])
	currentJob = 'deliver'
end

function goPickup()
	ESX.ShowNotification('Je kan nu je lading gaan ophalen (geel stipje op je navigatie).')
...
function getPaid()
	...
	if IsPedInAnyVehicle(GetPlayerPed(-1)) and isMyCar() then			
		...
		ESX.ShowNotification('~w~Vandaag verdient: ~g~ +' .. pay .. ' ~w~euro.')
	else														
		ESX.ShowNotification('~r~Heftruck 1')
		local pay = math.floor(packetsDelivered * (Config.pay * 0.6))			
		if packetsDelivered < 2 then						
			pay = 0								
		end
		TriggerServerEvent('esx_fork:getPaid', pay)
		ESX.ShowNotification('~w~Vandaag verdient: 2 ~r~ +' .. amount .. ' ~w~euro.')
	end
	currentJob = 'none'											
	currentPlate = ''												
	currentVehicle = nil											
	...
	while true do																
		...
		if not menuIsOpen then
			...
			if playerData.job ~= nil and playerData.job.name == "fork" and playerIsInside(playerCoords, Config.locker, 2.5) then 				
				isInMarker = true
				displayHint = true																
				hintToDisplay = "Druk op E om de kleedkamer in te gaan"									
				currentZone = 'locker'																
			elseif onDuty and taskPoints['deliver'] == nil and playerIsInside(playerCoords, Config.carSpawner, 2.5) then	
				isInMarker = true
				displayHint = true
				hintToDisplay = "Druk op E om je heftruck te pakken"
				currentZone = 'start'
			elseif onDuty and currentJob == 'deliver' and taskPoints['deliver'] ~= nil and playerIsInside(playerCoords, taskPoints['deliver'], Config.pickupDistance) then
				isInMarker = true
				displayHint = true
				hintToDisplay = "Zet hier je lading neer en ga naar de volgende."
				currentZone = 'none' 
			elseif playerData.job ~= nil and playerData.job.name == "fork" and currentPlate ~= '' and playerIsInside(playerCoords, Config.carDelete, 1.5) then  				
				isInMarker = true
				displayHint = true
				hintToDisplay = "Druk op E om je heftruck in te leveren"
				currentZone = 'pay'
			else																			
				isInMarker = false
				displayHint = false
...
function openMenu()									
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'locker',			
    {
      title    = "In/Uitklokken",							
      elements = {
        {label = "In dienst", value = 'fork_wear'},		
        {label = "Uit dienst", value = 'everyday_wear'}	
      }
    },
    function(data, menu)									
