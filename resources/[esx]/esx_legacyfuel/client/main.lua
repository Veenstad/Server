		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Stap uit je voertuig om te tanken")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Druk op ~g~G ~w~om te stoppen met tanken. €~r~" .. price .. " ~w~+  BTW")
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Voertuig is al vol, er kan niet getanked worden")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Je hebt niet genoeg geld op zak om brandstof te kopen")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Druk op ~g~G ~w~om te tanken. €~r~0.5/~w~liter + BTW")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Druk op ~g~G ~w~om te stoppen met tanken. Momenteel op: " .. fuel .. "% - Jerry Can: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Druk op ~g~G ~w~om het voertuig met uw jerrycan van brandstof te voorzien")
...
function DisplayHud()
	if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetSeatPedIsIn(GetPlayerPed(-1)) == -1 then
		...
		DrawAdvancedText(0.152 - x, 0.89 - y, 0.080, 0.0038, 0.6, fuel, 255, 255, 255, 255, 6, 1)

		--DrawAdvancedText(0.130 - x, 0.77 - y, 0.005, 0.0028, 0.6, mph, 255, 255, 255, 255, 6, 1)
		--DrawAdvancedText(0.174 - x, 0.77 - y, 0.005, 0.0028, 0.6, kmh, 255, 255, 255, 255, 6, 1)

		DrawAdvancedText(0.142 - x, 0.92 - y, 0.080, 0.0038, 0.4, "% Brandstof", 255, 255, 255, 255, 6, 1)
	end
end
