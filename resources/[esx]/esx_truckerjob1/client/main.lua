...
function IsJobTrucker()
	if PlayerData ~= nil then
		...
		if PlayerData.job.name ~= nil and PlayerData.job.name == 'trucker1' then
			isJobTrucker = true
		end
		return isJobTrucker
	end
...
AddEventHandler('esx_truckerjob1:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_truckerjob1:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()    
    CurrentAction = nil
    CurrentActionMsg = ''
end)
...
function donnerlapaye()
	...
	if vievehicule >= 1 then
		if livraisonTotalPaye == 0 then
			...
			TriggerServerEvent("esx_truckerjob1:pay", amount)
	...
	while true do
		...
		if IsJobTrucker() then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_truckerjob1:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('esx_truckerjob1:hasExitedMarker', lastZone)
			end
