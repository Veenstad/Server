...
if closestPlayer == -1 or closestDistance > 2.0 then 
    ESX.ShowNotification('~r~Er is geen speler vlakbij')
else
  if not HaveBagOnHead then
    TriggerServerEvent('esx_worek:sendclosest', GetPlayerServerId(closestPlayer))
    ESX.ShowNotification('~g~Zet de zak over het hoofd van ~w~' .. GetPlayerName(closestPlayer))
    TriggerServerEvent('esx_worek:closest')
  else
    ESX.ShowNotification('~r~Deze persoon heeft al een zak over zijn hoofd')
  end
end
...
AddEventHandler('esx_worek:zdejmijc', function(gracz)
    ESX.ShowNotification('~g~Iemand heeft de zak van jou hoofd verwijderd!')
    DeleteEntity(Worek)
    SetEntityAsNoLongerNeeded(Worek)
    SendNUIMessage({type = 'closeAll'})
    HaveBagOnHead = false
...
function OpenBagMenu() --This function is menu function
    ...
    local elements = {
          {label = 'Zet een zak op hoofd', value = 'puton'},
          {label = 'verwijder zak van hoofd', value = 'putoff'},
