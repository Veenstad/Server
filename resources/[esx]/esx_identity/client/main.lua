...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITEIT]', '^1Je moet eerst een karakter aanmaken voordat je spelen kan' } })
	end
end)
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Je karakternaam is te lang of te kort.'
	end
	...
	if count ~= nameLength then
		return 'Je karakternaam bevat een symbool dat niet is toegestaan op deze server.'
	end
	...
	if spacesInName > 2 then
		return 'Je naam bevat meer dan 2 spaties'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'Je naam moet met een hoofdletter beginnen.'
	end
