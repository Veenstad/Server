  ...
  local identifier =  ESX.GetPlayerFromId(src).identifier
        ...
        function(data)
            ...
            if count > data[1].count then
                ...
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Je kan niet meer meenemen dan je hebt!')
                else
      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Je hebt dit artikel al in je winkel, ~r~maar dan voor ' .. data[1].price .. '. terwijl je het bedrag wil zetten op ' .. Price)
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Verwijder het artikel en plaats het terug voor dezelfde prijs.')
            end
        end)  
    end)
end)
  ...
  local xPlayer = ESX.GetPlayerFromId(src)
        ...
        'SELECT * FROM owned_shops WHERE ShopNumber = @Number',
        {
            ...
            if xPlayer.getMoney() < ItemCount * result[1].price then
                TriggerClientEvent('esx:showNotification', src, '~r~Je hebt niet genoeg geld op zak.')
            elseif ItemCount <= 0 then
                TriggerClientEvent('esx:showNotification', src, '~r~Onjuiste hoeveelheid.')
            else
                xPlayer.removeMoney(ItemCount * result[1].price)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Je kocht ' .. ItemCount .. 'x ' .. Item .. ' voor €' .. ItemCount * result[1].price)
  ...
  local identifier = ESX.GetPlayerFromId(_source).identifier
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',{['@ShopNumber'] = id,['@identifier'] = identifier,}, function(result)
        ...
        if result[1].money >= price * count then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g~Je bestelde' .. count .. ' stuks ' .. label .. ' voor €' .. price * count)
        else
            TriggerClientEvent('esx:showNotification', _source, '~r~Je hebt niet genoeg geld in je kluis zitten.')
        end
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~gJe kocht een winkel voor $' ..  price)
        else    
            TriggerClientEvent('esx:showNotification', _source, '~r~Je kan deze winkel niet betalen')
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        ...
        if os.time() - result[1].LastRobbery <= 900 then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Je winkelkluis is gesloten in verband met een overval, wacht ' .. math.floor((900 - time) / 60) .. ' minuten')
            return
        end
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Je hebt €' .. amount .. ' in je kluis gestopt')
        else
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Je kan niet meer plaatsen dan je bezit')
        end
    end)
end)
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Je nam €' .. amount .. ' uit je winkelkluis')
    ...
    function(result)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx_kr_shops:removeBlip', -1)
            TriggerClientEvent('esx_kr_shops:setBlip', -1)
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Je hebt je winkel verkocht!')
            else
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Je kan je winkel niet verkopen wanneer er nog geld of producten in zit!')
            end
        end)
    end)
end)
