				...
				if CollectionAction == 'collection' then
					...
					if dist <= 3.5 then
						if currentbag > 0 then
							...
							trashcollection = false
							truckdeposit = false
							ESX.ShowNotification("De container is weer leeg, goed gewerkt!")
...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z,taillight_r.x - newposx , taillight_r.y - newposy, taillight_r.z)
			if dist <= 2.0 then
			ESX.Game.Utils.DrawText3D(vector3(taillight_r.x - newposx, taillight_r.y - newposy,taillight_r.z + 0.50), "[~g~E~s~] Gooi de zak in de wagen.", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag == 0 then
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Maak de container weer netjes.", 1.0)				
				else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Pak de vuilniszakken op en gooi ze achterin de wagen ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end
