		...
		if IsControlJustPressed(0, 51) and not onMission and IsPedInAnyVehicle(PlayerPedId()) then
			...
			if IsMissionVehicle(GetEntityModel(veh)) then
				...
				if time >= 0 and time <= TimeToSeconds(5, 30, 0) then
					...
					ShowMPMessage("Inbraak", "Vind een ~r~huis ~s~om daar intebreken.", 3500)
					--ShowSubtitle("Find a ~r~house ~s~ to rob")
				else
					DisplayHelpText("Inbraken kunnen alleen gedaan worden tussen 0:00 - 5:30.")
				end
			end
		end
...
CreateThread(function()
	while true do
		...
		if onMission then
			...
			-- draw info
			DrawTimerBar("Objecten", #stolenItems, 2)
			DrawTimerBar("Daglicht", AddLeadingZero(time.hours) .. ":" .. AddLeadingZero(time.minutes), 1)
		end
	end
end)
		...
		if onMission then		
			for k,door in pairs(doors) do
				...
				if Vdist(coords, door.coords.x, door.coords.y, door.coords.z) < 2 then
					DisplayHelpText("Druk op ~INPUT_CONTEXT~ om binnen naar tegaan.")
					...
					if IsControlJustPressed(0, 51) then
						...
						ShowSubtitle("Je bent nu binnen, pas op dat je ze niet wakker maakt, ze kunnen aggressief worden! (sneaking)")
					end
				end
			end
		end
