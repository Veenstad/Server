		...
		if isHolding then
			...
			if IsControlJustPressed(0, 51) then	
				...
				if isInHouse then
					...
					if Vdist(markerCoords, coords) < 3 then
						ShowSubtitle("Het object zit nu in je voertuig, ga terug naar het huis om meer te stelen.")
			...
			for _,pickup in pairs(pickups) do
				...
				if onMission and Vdist(coords, GetEntityCoords(pickup.prop)) < 2 and not isHolding then
					DisplayHelpText("Druk op ~INPUT_CONTEXT~ om te stelen")
					...
					if IsControlJustPressed(0, 51) then
						...
						ShowSubtitle("Je hebt wat op gepakt ga terug naar je voertuig om het in je bus te stoppen.")
