	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE plate=@plate",{['@plate'] = plate}, function(data) 
		for _,v in pairs(data) do
			...
			if (vehicle.model == model) then
				print('============')
				print('ANTI-CHEAT CHECK')
				print("Server-side =  " .. vehicle.model .. "")
				print("Client-side =  " .. model .. "")
				print("Check geslaagd!")
				print('============')
			else
				print('============')
				print('ANTI-CHEAT CHECK')
				print("Server-side =  " .. vehicle.model .. "")
				print("Client-side =  " .. model .. "")
				print('HACKER GEDECTEERD')
				print("Check Mislukt!")
				print('============')
				TriggerEvent('DiscordBot:ToDiscord', "https://discordapp.com/api/webhooks/564746581119729674/nAKCxL11OmEeum0JZW5j6T4s0aWTW1HG9R6yLD1RIjN9llcwqLO1LdhcEjEOYSkp7EnG" , xPlayer.identifier , "Persoon probeerde " .. model .. " inleveren maar het was daadwerkelijk een " .. vehicle.model, "https://previews.123rf.com/images/martialred/martialred1807/martialred180700028/105297939-ban-hammer-or-banhammer-to-block-users-flat-vector-icon-for-apps-and-websites.jpg", false, Source, TTS)
				TriggerEvent("banCheater", source,"Hackers worden hier niet geaccepteerd... Banned by Veenstad AntiCheat God")
			end
		end
	end)
...
function getPlayerVehicles(identifier)
	...
	for _,v in pairs(data) do
		local vehicle = json.decode(v.vehicle)
		table.insert(vehicles, {id = v.id, plate = v.plate, model = v.model})
	end
	return vehicles
end

function getPlayerVehicleModel(plate)
