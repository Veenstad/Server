...
mainMenu = NativeUI.CreateMenu("", "~s~BESTEMMING", "", "", "shopui_title_exec_vechupgrade", "shopui_title_exec_vechupgrade")
...
function AddAirPortMenu(menu)
	if config.use_essentialmode or config.use_venomous then
		SantonsButton = NativeUI.CreateItem("Veenstad Internationale luchthaven", "Koop een ticket voor "..config.moneyCurrency .." " ..config.ticketPrice)
		DesrtButton = NativeUI.CreateItem("Sandy Shores vliegveld", "Koop een ticket voor "..config.moneyCurrency .." " ..config.ticketPrice)
	else
		SantonsButton = NativeUI.CreateItem("Veenstad Internationale luchthaven", "")
		DesrtButton = NativeUI.CreateItem("Sandy Shores vliegveld", "")
	end
    ...
    menu.OnItemSelect = function(sender, item, index)
        if item == DesrtButton then
        	if not IsEntityInZone(PlayerPedId(), "DESRT") then
        		...
        		ShowNotification("Er is op het moment geen ~y~vlucht~w~ naar die locatie.")
        	end
